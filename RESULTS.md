...
### Leaderboard result — MIND Codabench 13967, submitted 2026-08-22 00:02
...
## Submission 2 · MIND weighted-sum fusion — leaderboard 2026-08-25
...
## Submission 3 · MIND point-in-time ranker — leaderboard 2026-08-25
...
## Gap-aware model selection — 2026-08-27
...
## Submission 4 · MIND, MiniLM semantics + skew-resistant features — 2026-08-27
...
### Submission 1 · BM25 — leaderboard 2026-08-25
...
## Q3 · Semantic retrieval on MIND — 2026-08-25
...
## Q3 (continued) · Semantic retrieval on EB-NeRD — 2026-08-30

```bash
PYTHONPATH=. .venv/bin/python -u scripts/eval_semantic_ebnerd.py --limit 20000 --recall-limit 3000
```

Unlike MIND, EB-NeRD **provides** article vectors, so none are computed here: Ekstra Bladet's own
word2vec (`document_vector.parquet`), 300-dimensional, publisher-trained on Danish news. Coverage
of the `ebnerd_small` corpus was verified before use and is **100.0%** (20,738 of 20,738) — there
is no unrepresentable-article caveat on this dataset, unlike the LSA path on MIND.
Log: `data/processed/semantic_ebnerd.log`; metrics: `data/processed/semantic_ebnerd.json`.

| Quantity | Value |
|---|---:|
| Embedding dimension | 300 |
| Embedding coverage | 100.0% |
| Embedding load | 4.2 s |
| FAISS `IndexFlatIP` build | 0.1 s |

### Q3.3 · User-vector pooling ablation — recall@K over 20,738 articles

3,000 validation impressions, all scored (no skips — every user has a representable history).

| Pooling | recall@50 | recall@100 | recall@200 |
|---|---:|---:|---:|
| mean | 0.00298 | 0.00663 | 0.01558 |
| **recency-weighted (τ=5)** | **0.00563** | **0.00928** | **0.01956** |

**This reverses the MIND result, and the reversal is the finding.** On MIND, mean pooling beat
recency-weighting at every K by roughly 40%, and RESULTS.md concluded that averaging more history
suppresses noise. On EB-NeRD the same ablation gives the opposite answer — recency-weighting wins
at every K, by 25–89%.

Both results are consistent with one mechanism, and it is the same mechanism that explains three
other EB-NeRD findings in this document. EB-NeRD is a **recency-dominated** dataset:
`age_hours` carries +0.1251 permutation importance there while every popularity feature sits near
+0.005 (Q9c), its candidate slates are pre-filtered by the publisher's own recommender to fresh
articles, and its test window sits seven to fourteen days past training. Where article freshness
is what separates candidates, a user vector tilted toward recent clicks describes the *current*
interest better than one averaged flat over the whole history. On MIND, where popularity and
content matter more relative to recency, the noise-suppression argument wins instead.

**Neither table carries a bootstrap CI**, so the same caution stated for the MIND `n_recent`
ablation applies: these are point estimates and the smaller gaps should not be read as certain.
The EB-NeRD gap is consistent in direction across all three K values, which the MIND
crossover-that-reversed was not.

### Q3.5 · Lexical vs semantic, by slice — in-impression AUC with 95% CI

20,000 validation impressions. Cold-start: ≤5 history clicks. Head/tail: whether the clicked
article is in the top popularity quintile of `ebnerd_small` train.

| Slice | n | BM25 (lexical) | word2vec (semantic) | Winner |
|---|---:|---|---|---|
| all | 20,000 | 0.5038 [0.4999, 0.5079] | 0.5056 [0.5010, 0.5106] | — overlap, no difference |
| cold-start (≤5 clicks) | 92 | 0.5196 [0.4628, 0.5756] | 0.4685 [0.4044, 0.5347] | — overlap, no difference |
| warm (>5 clicks) | 19,908 | 0.5037 [0.5001, 0.5074] | 0.5058 [0.5011, 0.5102] | — overlap, no difference |
| **head articles** | 444 | 0.5243 [0.5002, 0.5493] | **0.5815 [0.5497, 0.6114]** | **semantic**, disjoint |
| tail articles | 19,556 | 0.5033 [0.4995, 0.5072] | 0.5039 [0.4993, 0.5082] | — overlap, no difference |

**One slice separates, and it is the same slice that separated on MIND.** Semantic beats lexical
on **head articles** on both datasets — 0.5815 vs 0.5243 here, 0.5604 vs 0.5159 there. Popular
stories are covered by many near-synonymous headlines, which is exactly the case where term
overlap fails and a latent space does not. That this replicates across two languages, two
embedding sources (computed LSA vs provided word2vec) and two publishers is the strongest
cross-dataset result in this project.

**Everything else overlaps, including cold-start again.** Both models sit near chance overall
(0.504–0.506) on EB-NeRD, consistent with the Q2 finding that content similarity is a weak
predictor here relative to recency. The cold-start slice has only **n=92** — EB-NeRD users
almost all carry long histories — so that row is underpowered and no conclusion is drawn from it
beyond the interval being wide.

### Q4.2 · Beyond-accuracy (top-10 per impression)

| Metric | BM25 | Semantic | *MIND, for comparison* |
|---|---:|---:|---:|
| Intra-list diversity | 0.7971 | 0.7909 | *0.8474 / 0.8619* |
| Novelty (bits) | 17.082 | 17.055 | *16.192 / 16.199* |
| Catalogue coverage | 0.1240 | 0.1228 | *0.0388 / 0.0345* |
| Gini (exposure concentration) | 0.7043 | 0.7151 | *0.9056 / 0.9134* |

**EB-NeRD's exposure profile is materially healthier than MIND's** — 12.4% catalogue coverage
against 3.9%, and Gini 0.70 against 0.91. The likely reason is structural rather than a property
of our models: EB-NeRD's slates are already pre-filtered to a fresh, narrower candidate pool, so
the top-10 of an impression is drawn from a set that is itself closer to uniform. The MIND
warning still stands on its own terms — under 4% coverage there — but it should not be
generalised to "this pipeline is a popularity amplifier everywhere," because on EB-NeRD it
measurably is not.

## Q9 · Metrics with and without serving-unavailable features — 2026-08-25
