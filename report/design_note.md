...
| RecSys 2024 / EB-NeRD (2469), uploaded 2026-09-17 | `ebnerd_reranker_final.zip` | 0.6734 | **not scored by Codabench** — status "Submitted" from upload (17 Sep) through the time of writing (19 Sep), never "Running" or "Finished"; see below | — | 0.5110 |

**EB-NeRD: submitted, not scored — a platform problem, not a file problem.** The archive was uploaded to competition 2469 on 17 September and has sat at status "Submitted" for more than 48 hours without entering "Running"; A1's entry to the same competition was scored in 2.4 h in August. The file itself passed every check we control: it is the format A1's accepted submission used (`predictions.txt` at the archive root — the name SPEC §6 records for this competition), its sha256 equals the one the Kaggle kernel printed, and `validate_file` re-run locally over all 13,536,710 lines found every rank list a valid permutation of 1..N, one line per test impression and exactly the 200,000 shared-id beyond-accuracy rows the split defines. The MIND archive, produced by the same driver, scored normally on the same day. We therefore report the EB-NeRD leaderboard cell as unavailable at submission time and, as the nearest measured stand-in, the offline validation-week number (0.6734) with A1's measured dev-to-leaderboard offset for feature models (−0.03 to −0.09, SPEC §7), i.e. an expected leaderboard range of roughly 0.58–0.64. If Codabench scores the entry after this note is submitted, the number will be added to `RESULTS.md` Q5.6 in the repository.
...
<figure><img src="leaderboard_ebnerd.png" alt="EB-NeRD leaderboard"><figcaption>Figure 2. RecSys 2024 / EB-NeRD (Codabench 2469): the entry as it stood on 19 September — status "Submitted" since 17 September, never scored.</figcaption></figure>
