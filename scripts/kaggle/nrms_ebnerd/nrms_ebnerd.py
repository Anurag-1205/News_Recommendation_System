"""P3.1 · NRMS baseline on EB-NeRD: train on the shipped train split, score every validation
impression on its full slate, write the scores file, evaluate (SPEC.md §13).

Push:  .venv/bin/kaggle kernels push -p scripts/kaggle/nrms_ebnerd --accelerator NvidiaTeslaT4
Logs:  .venv/bin/kaggle kernels logs aayushpandey18602/a2-nrms-ebnerd > data/logs/kaggle/<name>.log
Files: .venv/bin/kaggle kernels output aayushpandey18602/a2-nrms-ebnerd -p data/scores/_kaggle/
...
df_hold = df.filter(pl.col(DEFAULT_IMPRESSION_TIMESTAMP_COL).dt.date() >= last_dt)
...
def build(emb):
    ...
    m = NRMSModel(hparams=hparams_nrms, word2vec_embedding=emb, seed=SEED_MODEL)
    m.model.compile(optimizer=m.model.optimizer, loss=m.model.loss, metrics=[tf.keras.metrics.AUC(name="auc")])
    return m


def loader(beh, eval_mode):
    cls = NRMSDataLoader if eval_mode else NRMSDataLoaderPretransform
    return cls(behaviors=beh, article_dict=article_mapping, unknown_representation="zeros",
               history_column=DEFAULT_HISTORY_ARTICLE_ID_COL, eval_mode=eval_mode, batch_size=BS)
...
# ---- 5. train ---------------------------------------------------------------------------------------
WEIGHTS = OUT / "weights" / "nrms.weights.h5"; WEIGHTS.parent.mkdir(parents=True, exist_ok=True)
callbacks = [
...
# ---- 6. score every validation impression, full slate, resumable chunks ----------------------------
CH = OUT / "chunks"; CH.mkdir(parents=True, exist_ok=True)
t = time.time()
for start in range(0, val_full.height, SCORE_CHUNK):
    ...
    scores = model.scorer.predict(loader(piece, True), verbose=0)
    piece = add_prediction_scores(piece, scores.tolist())
    piece.select("imp_row", "scores").write_parquet(f)
    print(f"chunk {f.name} {piece.height} impressions {time.time() - t:.0f}s", flush=True)
...
assert sf.height == int(val_full["article_ids_inview"].list.len().sum()), "rows != Σ slate lengths"
assert sf["imp_row"].n_unique() == val_full.height, "an imp_row is missing"
SC = OUT / "scores" / "ebnerd" / "validation"; SC.mkdir(parents=True, exist_ok=True)
sf.write_parquet(SC / "nrms.parquet")
manifest = {"dataset": "ebnerd", "split": f"{DATASPLIT}/validation", "system": "nrms", "framing": "in-impression",
            ...
            "command": "kaggle kernels push -p scripts/kaggle/nrms_ebnerd --accelerator NvidiaTeslaT4", "written_at": dt.datetime.utcnow().isoformat()}
(SC / "nrms.json").write_text(json.dumps(manifest, indent=2))
print("SCORES", sf.height, "rows", SC / "nrms.parquet")
stage("score")

# ---- 7. evaluate: ours (the reranker's code) and the benchmark's, must agree --------------------------
...
rows = impression_rows(joined, "score", label_col="labels")
per_imp = {k: np.asarray(v, float) for k, v in per_impression_metrics(rows).items()}
...
for k_ours, k_bench in [("auc", "auc"), ("mrr", "mrr"), ("ndcg@5", "ndcg@5"), ("ndcg@10", "ndcg@10")]:
